...
class Chef::Handler::Sentry < Chef::Handler
  def initialize(options = {})
    Raven.configure do |config|
      # We need to be able to lazily evaluate the node object inside
      # of a lambda so that we get any overrides that happen prior to
      # compile time.
      config.dsn = if options[:dsn].is_a?(Chef::DelayedEvaluator)
                     options[:dsn].call
                   else
                     options[:dsn]
                   end
    ...
    if options[:dsn]
      ...
      Raven.logger.warn "node['sentry']['dsn'] is not set! Raven will not send errors to Sentry!"
    end
  end
  ...
  def report
    ...
    event.extra = { run_id: run_status.run_id }
